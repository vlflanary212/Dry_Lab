...
# Date: 250127
# Objective: Annotate clusters derived from Seurat V5 processing of the NBAtlas.
...
# Downsample so all clusters are the same size as the smallest cluster
seurat_subset <- subset(
  seurat_obj,
  cells = WhichCells(seurat_obj, downsample = min(table(seurat_obj@active.ident)))
)
table(seurat_subset@active.ident)

# DEA
## Compute cluster-specific DEGs
