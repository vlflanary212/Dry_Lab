...
# Date: 250126
# Objective: Run the Seurat V5 workflow on the filtered NBAtlas dataset.
...
# Set filepaths
alldata_dir <- here("NB_ITH", "NBAtlas", "data", "alldata")
data_dir <- here("NB_ITH", "NBAtlas", "data", "neuroendocrine")
results_dir <- here("NB_ITH", "NBAtlas", "04_neuroendocrine")

# Load data
seurat_obj <- readRDS(here(alldata_dir, "01_filt_obj.rds"))

# Subset to only include neuroendocrine cells
seurat_obj <- subset(seurat_obj, idents = "Neuroendocrine")

# Split the data into layers by batch
## Add new metadata column for each batch (Study+Assay+Platform+Cell_condition)
...
## Split the Seurat object into layers
...
# Determine the number of calculated PCs that capture at least 90% of the variance
...
npcs <- which(cumulative_variance >= 90)[1]
...
seurat_clust <- FindNeighbors(
  seurat_obj, reduction = "pca", dims = 1:npcs, verbose = TRUE
  )
